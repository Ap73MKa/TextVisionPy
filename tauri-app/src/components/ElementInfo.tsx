...
export default function ElementInfo(props: {
    ...
    return (
        <div class="w-full h-full p-2">
            <div class="flex items-center justify-center w-full h-full rounded-2xl bg-white relative">
                {props.selectedRecord ? (
                    <>
                        <img src={props.selectedRecord.dataURL} alt="" class="h-40 w-40"/>
                        <button type="button" onClick={handleDeleteRecord} class="bg-red-400 top-0 right-0 mt-3 mr-3 w-36 h-7 hover:bg-red-500 transition-colors active:bg-red-600 rounded-2xl shadow-md text-white absolute">Delete record</button>
                    </>
                ) : (
